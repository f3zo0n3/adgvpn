...
create_dir() {
  ...
  if [ $? -eq 1 ];
  then
    echo "Starting root to create directory '$output_dir'"
    if root mkdir -p "$output_dir"; then
        root chown -R "${root_USER:-$USER}" "$output_dir"
        log "'$output_dir' has been created and ownership has been set to '${root_USER:-$USER}'"
    else
        error_exit "Failed to create '$output_dir' with root"
    fi
  else
...
check_owner() {
  ...
  then
    ...
    case "$response" in
    [yY]|[yY][eE][sS])
      target_user="${root_USER:-$USER}"
      if root chown -R "$target_user" "$output_dir"; then
...
unpack() {
  ...
  if [ "${symlink_exists}" -eq 1 ]; then
    ...
    [yY]|[yY][eE][sS])
      ...
      if ln -sf "${absolute_path}" /usr/local/bin 2> /dev/null || root ln -sf "${absolute_path}" /usr/local/bin; then
        symlink_exists='1'
...
handle_uninstall() {
  ...
  then
    ...
    if [ $? -eq 1 ];
    then
      echo "Starting root to remove '${output_dir}'"
      if root rmdir "${output_dir}"; then
        log "Empty directory '${output_dir}' has been removed"
      else
        error_exit "Failed to remove empty directory '${output_dir}' with root"
      fi
    else
      log "Empty directory '${output_dir}' has been removed"
    fi
  ...
  then
    ...
    if [ $? -eq 1 ];
    then
      echo "Starting root to remove '/usr/local/bin/${exe_name}'"
      if root rm -f "/usr/local/bin/${exe_name}"; then
        log "Symlink has been removed from '/usr/local/bin'"
      else
        log "Failed to remove symlink from '/usr/local/bin' with root"
      fi
    else
      log "Symlink has been removed from '/usr/local/bin'"
    fi
...
download() {
  ...
  if ! touch "$tmp_file" > /dev/null 2>&1; then
    ...
    case "$response" in
    [yY]|[yY][eE][sS])
      remove_command="root rm -f"
      if ! root curl -fsSL "$url" -o "$pkg_name"; then
